...
print "Before loading eForge::eStats\n";
print "Search directories:\n", join("\n", @INC), "\n";
use eForge::eStats;
print "After loading eForge::eStats\n";
use eForge::ePlot;
use eForge::eForge;
use Data::UUID;
...
foreach my $cell (sort {ncmp($$samples{$a}{'tissue'},$$samples{$b}{'tissue'}) || ncmp($a,$b)} @$cells){
    ...
    push(@results, [$teststat, $total_num_overlaps_in_random_picks, $zscore, $pbinom, $shortcell, $$samples{$cell}{'tissue'}, $$samples{$cell}{'datatype'}, $$samples{$cell}{'file'}, $dmp_string, $$samples{$cell}{'acc'}]);
}
if (!$web) {
    close(BACKGROUND);
}
...
print TSV join("\t", "TestStat", "Random-set-Overlap", "Zscore", "Pvalue", "Cell", "Tissue", "Datatype", "File", "Probe", "Accession", "Qvalue"), "\n";
